...
const LAYOUTS: Record<LayoutType, LayoutConfig> = {
  economic: {
    label: 'Ekonomik · 3 sütun × 9 satır', perSheet: 27, width: '64.5mm', height: '32mm', cols: 3, rows: 9,
    nameSize: 8.2, priceSize: 20.5, barcodeHeight: 18.5, barcodeWidth: 1.72, barcodeFontSize: 7.2, dateSize: 5.4,
  },
  compact: {
    label: '80 × 30 mm', perSheet: 18, width: '80mm', height: '30mm', cols: 2, rows: 9,
    nameSize: 10, priceSize: 25, barcodeHeight: 21, barcodeWidth: 1.65, barcodeFontSize: 7.8, dateSize: 6.6,
  },
  standard: {
    label: '100 × 38 mm', perSheet: 14, width: '100mm', height: '38mm', cols: 2, rows: 7,
    nameSize: 11.3, priceSize: 30, barcodeHeight: 24, barcodeWidth: 1.9, barcodeFontSize: 8.4, dateSize: 7, 
  },
  large: {
    label: '100 × 50 mm', perSheet: 10, width: '100mm', height: '50mm', cols: 2, rows: 5,
    nameSize: 13, priceSize: 38.5, barcodeHeight: 30, barcodeWidth: 2.2, barcodeFontSize: 9.4, dateSize: 7.2,
  },
};
...
function PrintLayout({ selected, config }: { selected: SelectedProduct[]; config: LayoutConfig }) {
  ...
  return (
    <div style={{ display: 'grid', gridTemplateColumns: `repeat(${config.cols}, ${config.width})`, gridAutoRows: config.height, columnGap: config.cols === 3 ? '1mm' : '0.8mm', rowGap: '0.8mm', width: config.cols === 3 ? '195.5mm' : '200.8mm' }}>
...
function LabelContent({ item, config, svgRef, scale = 1 }: { item: SelectedProduct; config: LayoutConfig; svgRef: React.RefObject<SVGSVGElement>; scale?: number }) {
  ...
  return (
    <div style={{ display: 'flex', height: '100%', flexDirection: 'column', color: '#111', fontFamily: 'Arial, Helvetica, sans-serif' }}>
      <div style={{ display: 'flex', alignItems: 'baseline', justifyContent: 'space-between', gap: s(1), lineHeight: 1 }}>
        <div style={{ fontSize: s(4.9), fontWeight: 900, letterSpacing: '0.08em', whiteSpace: 'nowrap' }}>FERHAT BÜFE</div>
        <div style={{ fontSize: s(config.nameSize), fontWeight: 900, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis', flex: 1, textAlign: 'right' }}>{item.product.name}</div>
      </div>
      <div style={{ marginTop: s(1), fontSize: s(6.3), fontWeight: 800, letterSpacing: '0.04em' }}>KDV DAHİL SATIŞ FİYATI</div>

      <div style={{ display: 'flex', flex: 1, minHeight: 0, alignItems: 'center', justifyContent: 'space-between', gap: s(2) }}>
        <div style={{ minWidth: 0, width: discounted ? '43%' : '36%', flexShrink: 0, display: 'flex', flexDirection: 'column', justifyContent: 'center', alignItems: 'flex-start' }}>
          {discounted ? (
            <>
              <div style={{ display: 'flex', alignItems: 'baseline', gap: s(1), lineHeight: 1, whiteSpace: 'nowrap' }}>
                <span style={{ fontSize: s(6.2), fontWeight: 900 }}>ÖNCE</span>
                <span style={{ fontSize: s(config.priceSize * 0.48), fontWeight: 900, textDecoration: 'line-through', textDecorationThickness: `${Math.max(1, s(1))}px`, textDecorationSkipInk: 'none' }}>{formatCurrency(item.oldPrice)}</span>
              </div>
              <div style={{ marginTop: s(1), fontSize: s(6.5), fontWeight: 900, lineHeight: 1, whiteSpace: 'nowrap' }}>{`%${percent} İNDİRİM`}</div>
              <div style={{ marginTop: s(0.8), fontSize: s(config.priceSize), fontWeight: 900, letterSpacing: '-0.7px', lineHeight: 0.9, whiteSpace: 'nowrap' }}>{formatCurrency(price)}</div>
            </>
          ) : (
            <div style={{ fontSize: s(config.priceSize), fontWeight: 900, letterSpacing: '-0.7px', lineHeight: 0.9, whiteSpace: 'nowrap' }}>{formatCurrency(price)}</div>
          )}
        </div>

        <svg ref={svgRef} style={{ width: discounted ? '54%' : '62%', height: s(config.barcodeHeight), display: 'block', flexShrink: 0 }} />
      </div>
