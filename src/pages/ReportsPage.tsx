...
export default function ReportsPage() {
  ...
  const [saleCosts, setSaleCosts] = useState<Record<string, number>>({});
  ...
  useEffect(() => {
    ...
    const loadCosts = async () => {
      ...
      const { data, error } = await supabase
        .from('sale_items')
        .select('sale_id, quantity, unit_cost, product_id')
        .in('sale_id', saleIds);

      let rows = data as Array<{ sale_id: string; quantity: number; unit_cost: number | null; product_id: string | null }> | null;

      // Eski veritabanlarında unit_cost yoksa raporları bozma. Eski satışlar için
      // mevcut ürün maliyetini yalnızca geri dönüş/fallback olarak kullan.
      if (error && /unit_cost|schema cache|column/i.test(error.message || '')) {
        const fallback = await supabase
          .from('sale_items')
          .select('sale_id, quantity, product_id')
          .in('sale_id', saleIds);
        if (!fallback.error) {
          const productIds = [...new Set((fallback.data || []).map((r: any) => r.product_id).filter(Boolean))];
          const productsById: Record<string, number> = {};
          if (productIds.length) {
            const pr = await supabase.from('products').select('id, cost').in('id', productIds);
            (pr.data || []).forEach((p: any) => { productsById[p.id] = Number(p.cost) || 0; });
          }
          rows = (fallback.data || []).map((r: any) => ({ ...r, unit_cost: productsById[r.product_id] ?? 0 }));
        }
      }
      ...
      if (!cancelled) {
        ...
        (rows || []).forEach((row) => {
          ...
          bySale[row.sale_id] = (bySale[row.sale_id] || 0) + qty * cost;
        });
        setSaleCosts(bySale);
  ...
  const stats = useMemo(() => {
    ...
    const cost = filteredSales.reduce((sum, s) => sum + (saleCosts[s.id] || 0), 0);
    const profit = total - cost;
    const margin = total > 0 ? (profit / total) * 100 : 0;
    const avg = count > 0 ? total / count : 0;
    return { total, cost, profit, margin, cashTotal, cardTotal, creditTotal, count, avg };
  }, [filteredSales, saleCosts]);
    ...
    <div className="flex h-screen flex-col overflow-hidden">
      ...
      <div className="flex-1 overflow-y-auto p-6">
        ...
        <div className="mb-6 grid gap-4 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-6">
          <StatCard icon={<TrendingUp className="text-teal-600" size={22} />} label="Toplam Ciro" value={formatCurrency(stats.total)} color="teal" />
          <StatCard icon={<Banknote className="text-red-600" size={22} />} label="Toplam Maliyet" value={costLoading ? 'Hesaplanıyor...' : formatCurrency(stats.cost)} color="red" />
          <StatCard icon={<TrendingUp className="text-emerald-600" size={22} />} label="Net Kâr" value={costLoading ? 'Hesaplanıyor...' : formatCurrency(stats.profit)} color="green" />
          <StatCard icon={<CreditCard className="text-violet-600" size={22} />} label="Kâr Marjı" value={costLoading ? 'Hesaplanıyor...' : `%${stats.margin.toFixed(1)}`} color="purple" />
          <StatCard icon={<ShoppingBag className="text-blue-600" size={22} />} label="Satış Adedi" value={stats.count.toString()} color="blue" />
          <StatCard icon={<Package className="text-amber-600" size={22} />} label="Ürün Çeşidi" value={products.length.toString()} color="amber" />
        </div>
            ...
            <table className="w-full">
              <thead className="bg-slate-50">
                <tr>
                  ...
                  <th className="px-4 py-2 text-right text-xs font-semibold text-slate-600">Kâr</th>
                ...
                {filteredSales.slice(0, 50).map((sale) => {
                  ...
                  return (
                    <tr key={sale.id} className="border-b border-slate-100 hover:bg-slate-50 cursor-pointer" onClick={() => setSelectedSale(sale)}>
                      ...
                      <td className="px-4 py-3 text-right font-semibold text-emerald-700">{formatCurrency(parseFloat(sale.total.toString()) - (saleCosts[sale.id] || 0))}</td>
                      <td className="px-4 py-3">
