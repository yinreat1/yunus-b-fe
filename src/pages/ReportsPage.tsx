...
import { TrendingUp, ShoppingBag, Banknote, CreditCard, BookOpen, Package, AlertTriangle, Calendar, ChevronRight, Trash2, RotateCcw, Trash, Receipt } from 'lucide-react';
import { getShopExpenses, getSaleItems, moveSaleToTrash, refundSale, partialReturn, useCashSessions, useDeletedSales, useProducts, useSales, useStaff, restoreSaleFromTrash, permanentlyDeleteSale } from '@/lib/hooks';
...
export default function ReportsPage() {
  const { sales, loading, reload: reloadSales } = useSales(50000);
  const { sales: deletedSales, loading: trashLoading, reload: reloadTrash } = useDeletedSales(200);
  const { products } = useProducts();
  const { staff } = useStaff();
  const staffMap = useMemo(() => new Map(staff.map((person) => [person.id, person.name])), [staff]);
  const { sessions: cashSessions } = useCashSessions(500);
  const [reportSaleItems, setReportSaleItems] = useState<SaleItem[]>([]);
  ...
  const filteredSales = useMemo(() => {
    let activeSales = sales.filter((s) => !s.refunded_at && !String(s.deleted_reason || '').startsWith('İADE|'));
    if (staffFilter !== 'all') activeSales = activeSales.filter((s) => staffFilter === '__unassigned__' ? !s.staff_id : s.staff_id === staffFilter);
    if (dateRange === 'all') return activeSales;
    const now = new Date();
    ...
    if (dateRange === 'today') {
      ...
      return activeSales.filter((s) => new Date(s.created_at) >= start && new Date(s.created_at) < end);
    } else if (dateRange === 'week') start.setDate(now.getDate() - 7);
    else if (dateRange === 'month') start.setMonth(now.getMonth() - 1);
    return activeSales.filter((s) => new Date(s.created_at) >= start);
  }, [sales, dateRange, staffFilter]);

  useEffect(() => {
    let cancelled = false;
    const ids = filteredSales.map((s) => s.id);
    if (!ids.length) { setReportSaleItems([]); return; }
    supabase.from('sale_items').select('*').in('sale_id', ids).then(({ data }) => { if (!cancelled) setReportSaleItems((data || []) as SaleItem[]); });
  ...
  const refundTotal = useMemo(() => {
    return sales
      .filter((s) => s.refunded_at || String(s.deleted_reason || '').startsWith('İADE|'))
      .filter((s) => {
        ...
        if (dateRange === 'today') {
          ...
          const refundDate = s.refunded_at || String(s.deleted_reason || '').split('|')[1] || s.created_at;
          return new Date(refundDate) >= start && new Date(refundDate) < end;
        } else if (dateRange === 'week') start.setDate(now.getDate() - 7);
        else if (dateRange === 'month') start.setMonth(now.getMonth() - 1);
        const refundDate = s.refunded_at || String(s.deleted_reason || '').split('|')[1] || s.created_at;
        return new Date(refundDate) >= start;
      })
      .reduce((sum, s) => sum + Number(s.refund_amount ?? (String(s.deleted_reason || '').startsWith('İADE|') ? String(s.deleted_reason).split('|')[2] : s.total) ?? 0), 0);
  }, [sales, dateRange]);
  ...
  const shopExpenses = useMemo(() => {
    ...
    else if (dateRange === 'yesterday') { start.setDate(now.getDate() - 1); start.setHours(0, 0, 0, 0); end = new Date(now); end.setHours(0, 0, 0, 0); }
    else if (dateRange === 'week') start.setDate(now.getDate() - 7);
    else if (dateRange === 'month') start.setMonth(now.getMonth() - 1);
  ...
  const stats = useMemo(() => {
    ...
    const cost = reportSaleItems.reduce((sum, item) => sum + Number(item.quantity || 0) * Number(costByProduct.get(item.product_id || '') || 0), 0);
    const shopExpense = shopExpenses.reduce((sum, item) => sum + Number(item.amount || 0), 0);
    const netProfit = total - cost - shopExpense;
    const margin = total > 0 ? (netProfit / total) * 100 : 0;
    const avg = count > 0 ? total / count : 0;
    return { total, cashTotal, cardTotal, creditTotal, count, avg, cost, shopExpense, netProfit, margin };
  }, [filteredSales, products, reportSaleItems, shopExpenses]);

  const productPerformance = useMemo(() => {
    const costs = new Map(products.map(p => [p.id, Number(p.cost || 0)]));
    const m = new Map<string,{name:string;qty:number;revenue:number;profit:number}>();
    for (const it of reportSaleItems) { const k=it.product_id || it.product_name; const cur=m.get(k)||{name:it.product_name,qty:0,revenue:0,profit:0}; const q=Number(it.quantity||0), rev=Number(it.subtotal||0); cur.qty+=q; cur.revenue+=rev; cur.profit+=rev-q*Number(costs.get(it.product_id||'')||0); m.set(k,cur); }
    return {best:[...m.values()].sort((a,b)=>b.qty-a.qty).slice(0,5),profit:[...m.values()].sort((a,b)=>b.profit-a.profit).slice(0,5)};
  }, [reportSaleItems, products]);
    ...
    <div className="flex h-screen flex-col overflow-hidden">
        ...
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-3">
            ...
            <select
              ...
              title="Raporu personele göre filtrele"
            >
              <option value="all">Tüm Personel</option>
              {staff.map((person) => <option key={person.id} value={person.id}>{person.name}</option>)}
              <option value="__unassigned__">Personel atanmamış</option>
            </select>
          </div>
        </div>
      </div>
      ...
      <div className="flex-1 overflow-y-auto p-6">
        ...
        <div className="mb-6 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
          ...
          />
          <StatCard
            icon={<RotateCcw className="text-rose-600" size={22} />}
            label="İade Tutarı"
            value={formatCurrency(refundTotal)}
            color="rose"
          />
          <StatCard icon={<TrendingUp className="text-emerald-600" size={22} />} label="Net Kâr" value={formatCurrency(stats.netProfit)} color="emerald" />
          <StatCard icon={<Receipt className="text-rose-600" size={22} />} label="Dükkan Gideri" value={formatCurrency(stats.shopExpense)} color="rose" />
          <StatCard
        ...
        <div className="mb-6 grid gap-4 lg:grid-cols-2">
          <section className="card overflow-hidden"><div className="border-b border-slate-200 px-4 py-3 font-bold">En Çok Satanlar</div><div className="divide-y divide-slate-100">{productPerformance.best.map((p,i)=><div key={i} className="flex items-center justify-between px-4 py-3 text-sm"><span>{i+1}. {p.name}</span><span className="font-bold text-teal-700">{p.qty} adet</span></div>)}{productPerformance.best.length===0&&<p className="p-5 text-sm text-slate-400">Veri yok.</p>}</div></section>
          <section className="card overflow-hidden"><div className="border-b border-slate-200 px-4 py-3 font-bold">En Çok Kazandıranlar</div><div className="divide-y divide-slate-100">{productPerformance.profit.map((p,i)=><div key={i} className="flex items-center justify-between px-4 py-3 text-sm"><span>{i+1}. {p.name}</span><span className="font-bold text-emerald-700">{formatCurrency(p.profit)}</span></div>)}{productPerformance.profit.length===0&&<p className="p-5 text-sm text-slate-400">Veri yok.</p>}</div></section>
        </div>
          ...
          {loading ? (
            ...
            <table className="w-full">
              <thead className="bg-slate-50">
                <tr>
                  <th className="px-4 py-2 text-left text-xs font-semibold text-slate-600">Personel / Tarih</th>
              ...
              <tbody>
                {filteredSales.slice(0, 50).map((sale) => {
                  const method = PAYMENT_METHODS[sale.payment_method as keyof typeof PAYMENT_METHODS] || PAYMENT_METHODS.cash;
                  const badgeClasses = sale.payment_method === 'card' ? 'bg-blue-100 text-blue-700' : sale.payment_method === 'credit' ? 'bg-amber-100 text-amber-700' : sale.payment_method === 'split' ? 'bg-violet-100 text-violet-700' : 'bg-emerald-100 text-emerald-700';
                  return (
                    <tr key={sale.id} className="border-b border-slate-100 hover:bg-slate-50 cursor-pointer" onClick={() => setSelectedSale(sale)}>
                      <td className="px-4 py-3">
                        <div className="text-sm font-bold text-slate-800">{sale.staff_id ? (staffMap.get(sale.staff_id) || 'Personel bilinmiyor') : 'Personel atanmamış'}</div>
                        <div className="mt-0.5 text-xs text-slate-500">{formatDateTime(sale.created_at)}</div>
                      </td>
                      <td className="px-4 py-3">
                        <span className={`inline-block rounded-full px-2 py-0.5 text-xs font-medium ${badgeClasses}`}>
                          {method.label}
                        </span>
                        {(sale.refunded_at || String(sale.deleted_reason || '').startsWith('İADE|')) && (
                          <span className="ml-1 inline-block rounded-full bg-amber-100 px-2 py-0.5 text-xs font-medium text-amber-700">İade</span>
                        )}
                      </td>
      ...
      {selectedSale && (
        <SaleDetailModal
          ...
          onRefund={async () => {
            setActionLoading(true);
            const ok = await refundSale(selectedSale.id);
            setActionLoading(false);
            if (ok) {
              setSelectedSale(null);
              await reloadSales();
            } else {
              alert("İade işlemi yapılamadı. Satış veya ürün kayıtlarına erişilemiyor.");
            }
          }}
                ...
                <div key={sale.id} className="flex items-center justify-between gap-4 px-5 py-4 hover:bg-slate-50">
                  <div className="min-w-0">
                    <div className="flex flex-wrap items-center gap-2">
                      <span className="rounded-full bg-teal-50 px-2 py-0.5 text-[11px] font-semibold text-teal-700">{sale.staff_id ? (staffMap.get(sale.staff_id) || 'Personel bilinmiyor') : 'Personel atanmamış'}</span>
                    ...
                    <p className="mt-1 text-xs text-slate-500">
                      {sale.payment_method === 'split' ? 'Karma' : PAYMENT_METHODS[sale.payment_method].label} · {sale.customer_name || 'Müşteri yok'} · {formatCurrency(Number(sale.total))}
                    </p>
...
function SaleDetailModal({
  ...
  const method = sale.payment_method === 'split' ? { label: 'Karma', color: 'violet' } : PAYMENT_METHODS[sale.payment_method];
  const [returningId, setReturningId] = useState<string | null>(null);
  const [returnQty, setReturnQty] = useState('1');
  async function doPartialReturn(item: SaleItem) { const qty=Math.max(0,Number(returnQty)||0); if(qty<=0)return; setReturningId(item.id); const ok=await partialReturn(sale.id,item.id,qty,'Kısmi iade'); setReturningId(null); if(ok){ const fresh=await getSaleItems(sale.id); setItems(fresh); } else alert('İade miktarı, satılabilir kalan miktarı aşamaz.'); } 
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 animate-fade-in" onClick={onClose}>
      <div className="card w-full max-w-md p-6 animate-slide-up" onClick={(e) => e.stopPropagation()}>
        <div className="mb-4 flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            {!(sale.refunded_at || String(sale.deleted_reason || '').startsWith('İADE|')) && (
              <button
                onClick={async () => {
                  if (window.confirm('Bu satışın TAMAMINI iade etmek istiyor musun? Ürünler stoğa geri eklenecek ve veresiye ise müşteri bakiyesi düşürülecek.')) {
                    await onRefund();
                  }
                }}
                disabled={actionLoading}
                className="flex items-center gap-1.5 rounded-md bg-amber-50 px-2.5 py-1.5 text-xs font-medium text-amber-700 hover:bg-amber-100 disabled:opacity-50"
              >
        ...
        <div className="mb-4 flex flex-wrap items-center justify-between gap-2 text-sm text-slate-500">
          <div>
            <div className="font-bold text-slate-800">{sale.staff_id ? (staffMap.get(sale.staff_id) || 'Personel bilinmiyor') : 'Personel atanmamış'}</div>
          ...
          {(sale.refunded_at || String(sale.deleted_reason || '').startsWith('İADE|')) && (
            <span className="rounded-full bg-amber-100 px-2 py-0.5 text-xs font-medium text-amber-700">İade edildi</span>
          )}
        </div>
          ...
          <div className="space-y-2 border-t border-slate-200 pt-3">
            {items.map((item) => (
              <div key={item.id} className="rounded-lg bg-slate-50 p-2 text-sm">
                <div className="flex justify-between"><span className="text-slate-600">{item.quantity}x {item.product_name}</span><span className="font-medium text-slate-700">{formatCurrency(parseFloat(item.subtotal.toString()))}</span></div>
                {!(sale.refunded_at || String(sale.deleted_reason || '').startsWith('İADE|')) && <div className="mt-2 flex items-center justify-end gap-2"><input type="number" min="0.001" max={item.quantity} step="0.001" className="h-8 w-20 rounded border border-slate-200 px-2 text-xs" value={returnQty} onChange={e=>setReturnQty(e.target.value)} /><button disabled={returningId===item.id} onClick={()=>doPartialReturn(item)} className="rounded-md bg-amber-50 px-2.5 py-1.5 text-xs font-semibold text-amber-700">{returningId===item.id?'...':'Kısmi İade'}</button></div>}
              </div>
            ))}
            ...
            {(sale.refunded_at || String(sale.deleted_reason || '').startsWith('İADE|')) && (
              <div className="rounded-md bg-amber-50 p-2 text-xs text-amber-700">
                İade edildi: {formatDateTime(sale.refunded_at || String(sale.deleted_reason || '').split('|')[1] || sale.created_at)} · {formatCurrency(Number(sale.refund_amount ?? (String(sale.deleted_reason || '').split('|')[2] || sale.total)))}
              </div>
            )}
          </div>
        )}
