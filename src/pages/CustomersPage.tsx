...
function shareCustomerDebtWhatsApp(customer: Customer, sales: any[], payments: any[], balance: number) {
  if (!customer.phone) { alert('WhatsApp göndermek için müşterinin telefon numarasını kaydedin.'); return; }
...
function CustomerDetail({ customer, onClose, onChanged }: { customer: Customer; onClose: () => void; onChanged: () => void }) {
  ...
  return <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4" onClick={onClose}>
    <div className="card flex max-h-[92vh] w-full max-w-5xl flex-col overflow-hidden" onClick={(e) => e.stopPropagation()}>
      <div className="flex items-start justify-between border-b border-slate-200 bg-white px-5 py-4"><div><h2 className="text-xl font-bold text-slate-800">{customer.name}</h2>{customer.phone && <p className="text-sm text-slate-500">{customer.phone}</p>}</div><div className="flex items-center gap-2"><button onClick={() => shareCustomerDebtWhatsApp(customer, sales, payments, currentBalance)} disabled={!customer.phone} className="inline-flex items-center gap-2 rounded-lg bg-emerald-600 px-3 py-2 text-xs font-bold text-white hover:bg-emerald-700 disabled:opacity-40"><MessageCircle size={16}/> WhatsApp Borç Özeti</button><button onClick={onClose} className="text-slate-400 hover:text-slate-600"><X size={22} /></button></div></div>
