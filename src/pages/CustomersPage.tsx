...
function shareCustomerDebtWhatsApp(customer: Customer, sales: any[], payments: any[], balance: number) {
  ...
  const phone = digits ? (digits.startsWith('0') ? `90${digits.slice(1)}` : digits.startsWith('90') ? digits : `90${digits}`) : ''; 
  const lines = [
    `*${customer.name} Borç Özeti*`, '',
    ...sales.flatMap((sale: any) => [
      `📅 ${formatDateTime(sale.created_at)}`,
      ...(sale.sale_items || []).map((item: any) => `• ${item.quantity} × ${item.product_name} — ${formatCurrency(item.subtotal)}`),
...
function CustomerDetail({ customer, onClose, onChanged }: { customer: Customer; onClose: () => void; onChanged: () => void }) {
  const { sales, archiveSales, payments, currentBalance: liveBalance, loading, reload } = useCustomerDebtHistory(customer.id);
  const [historyTab, setHistoryTab] = useState<'active'|'archive'>('active');
  const [payAmount, setPayAmount] = useState('');
  const [payNote, setPayNote] = useState('');
  const [payMode, setPayMode] = useState(false);
  const [saving, setSaving] = useState(false);
  const currentBalance = Math.max(0, Number(liveBalance ?? customer.balance ?? 0));
  const totalPurchases = [...sales, ...archiveSales].reduce((sum, sale) => sum + Number(sale.total || 0), 0);
  const openPurchases = sales.reduce((sum, sale) => sum + Number(sale.total || 0), 0);
  ...
  async function handlePayment() {
    ...
    try { staffId = (JSON.parse(sessionStorage.getItem('propos-current-staff-v2') || 'null') as any)?.id || undefined; } catch {}
    const result = await payCustomerDebt(customer.id, amount, payNote, staffId);
    setSaving(false);
  ...
  return <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4" onClick={onClose}>
    <div className="card flex max-h-[92vh] w-full max-w-5xl flex-col overflow-hidden" onClick={(e) => e.stopPropagation()}>
      <div className="flex items-start justify-between border-b border-slate-200 bg-white px-5 py-4"><div><h2 className="text-xl font-bold text-slate-800">{customer.name}</h2>{customer.phone && <p className="text-sm text-slate-500">{customer.phone}</p>}</div><div className="flex items-center gap-2"><button onClick={() => shareCustomerDebtWhatsApp(customer, sales, payments, currentBalance)} className="inline-flex items-center gap-2 rounded-lg bg-emerald-600 px-3 py-2 text-xs font-bold text-white hover:bg-emerald-700 disabled:opacity-40"><MessageCircle size={16}/> WhatsApp Borç Özeti</button><button onClick={onClose} className="text-slate-400 hover:text-slate-600"><X size={22} /></button></div></div>
      ...
      <div className="grid gap-3 border-b border-slate-200 bg-slate-50 p-4 sm:grid-cols-4">
        ...
        <Stat label="Toplam veresiye" value={formatCurrency(openPurchases)} accent="slate" />
      ...
      <div className="flex-1 overflow-y-auto p-4">
        ...
        <div className="mb-6">
          <div className="mb-3 flex items-center justify-between gap-3">
            <div className="flex items-center gap-2"><ReceiptText size={18} className="text-amber-600" /><h3 className="font-bold text-slate-800">Cari Geçmişi</h3></div>
            <div className="inline-flex rounded-lg bg-slate-100 p-1">
              <button onClick={() => setHistoryTab('active')} className={`rounded-md px-3 py-1.5 text-xs font-bold ${historyTab === 'active' ? 'bg-white text-amber-700 shadow-sm' : 'text-slate-500'}`}>Açık Hesap</button>
              <button onClick={() => setHistoryTab('archive')} className={`rounded-md px-3 py-1.5 text-xs font-bold ${historyTab === 'archive' ? 'bg-white text-blue-700 shadow-sm' : 'text-slate-500'}`}>Eski Sayfalar / Arşiv ({archiveSales.length})</button>
            </div>
          </div>
          {loading ? <p className="py-8 text-center text-slate-400">Geçmiş yükleniyor...</p> : (historyTab === 'active' ? sales : archiveSales).length === 0 ? <div className="rounded-xl border border-dashed border-slate-300 p-8 text-center text-sm text-slate-400">{historyTab === 'active' ? (currentBalance <= 0 ? 'Bu müşterinin açık borcu yok. Eski alışverişleri “Eski Sayfalar” bölümünde görebilirsiniz.' : 'Açık veresiye alışverişi bulunmuyor.') : 'Henüz kapanmış hesap sayfası yok.'}</div> : <div className="space-y-3">{(historyTab === 'active' ? sales : archiveSales).map((sale) => <SaleHistoryCard key={sale.id} sale={sale} archived={historyTab === 'archive'} />)}</div>}
        </div>
