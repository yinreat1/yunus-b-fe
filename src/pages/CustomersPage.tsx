...
import { Users, Plus, Search, Phone, Trash2, Edit, X, Wallet, TrendingUp, UserCheck, Clock, ReceiptText, CreditCard } from 'lucide-react';
...
function CustomerDetail({ customer, onClose, onChanged }: { customer: Customer; onClose: () => void; onChanged: () => void }) {
  ...
  return <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4" onClick={onClose}>
    <div className="card flex max-h-[92vh] w-full max-w-5xl flex-col overflow-hidden" onClick={(e) => e.stopPropagation()}>
      <div className="flex items-start justify-between border-b border-slate-200 bg-white px-5 py-4"><div><h2 className="text-xl font-bold text-slate-800">{customer.name}</h2>{customer.phone && <p className="text-sm text-slate-500">{customer.phone}</p>}</div><button onClick={onClose} className="text-slate-400 hover:text-slate-600"><X size={22} /></button></div>
