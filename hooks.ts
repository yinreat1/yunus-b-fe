...
import type { Category, Product, Sale, SaleItem, SaleWithItems, Customer, CustomerPayment, CashSession, SalePayment, StockMovement, Staff, StaffEarning } from '@/lib/supabase';
...
async function recordImmediateStaffEarning(params: {
  ...
  // Satışı yapan personeli de satış kaydına bağla; Kazancım ekranı bu bağlantıyı kullanır.
  await supabase.from('sales').update({ staff_id: resolvedStaff.id }).eq('id', saleId);
  ...
  try {
    ...
    if (!atomicError && atomicSaleId) {
      ...
      if (!atomicSaleError && atomicSale) {
        ...
        if (immediatePaid > 0) {
          await recordImmediateStaffEarning({
            saleId: atomicSale.id,
